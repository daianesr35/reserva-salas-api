...
public class ReservaService {
    ...
    public Reserva criar(Long salaId, Long usuarioId,
        ...
        if (conflito) throw new IllegalStateException("Conflito de horário para a sala");
    ...
    public Reserva atualizar(Long reservaId, Long usuarioId,
        ...
        if (!r.getUsuario().getId().equals(usuarioId)) {
            throw new SecurityException("Não pode editar reserva de outro usuário");
        }

        boolean conflito = reservaRepository.existsConflito(r.getSala().getId(), data, inicio, fim);

        // Se o usuário realmente mudou data/horário, e há conflito, bloqueia
        boolean mudou = !data.equals(r.getData()) || !inicio.equals(r.getHoraInicio()) || !fim.equals(r.getHoraFim());
        if (mudou && conflito) throw new IllegalStateException("Conflito de horário para a sala");
    ...
    public void cancelar(Long reservaId, Long usuarioId) {
        ...
        if (!r.getUsuario().getId().equals(usuarioId)) {
            throw new SecurityException("Não pode cancelar reserva de outro usuário");
        }
    ...
    private void validarIntervalo(LocalTime inicio, LocalTime fim) {
        if (!inicio.isBefore(fim)) {
            throw new IllegalArgumentException("Hora início deve ser antes da hora fim");
        }
        // (opcional) regras como: só horário comercial, múltiplos de 30 min etc.
    }
}
